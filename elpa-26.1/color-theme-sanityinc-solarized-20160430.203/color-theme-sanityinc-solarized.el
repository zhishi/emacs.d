...
`MODE' should be set to either 'light or 'dark."
  ...
  `(let* ((srgb color-theme-sanityinc-solarized-rgb-is-srgb)
          (base03  (if (or (eq 'tlight ,mode) (eq 'tdark ,mode)) "#1c1c1c"
                     (if srgb "#002b36" "#042028"))) ; (0.0159 0.1265 0.1597)
          (base02  (if (or (eq 'tlight ,mode) (eq 'tdark ,mode)) "#262626"
                     (if srgb "#073642" "#0a2832"))) ; (0.0394 0.1601 0.1983)
          (base01  (if (or (eq 'tlight ,mode) (eq 'tdark ,mode)) "#585858"
                     (if srgb "#586e75" "#465a61"))) ; (0.2767 0.3567 0.3830)
          (base00  (if (or (eq 'tlight ,mode) (eq 'tdark ,mode)) "#626262"
                     (if srgb "#657b83" "#52676f"))) ; (0.3244 0.4072 0.4385)
          (base0   (if (or (eq 'tlight ,mode) (eq 'tdark ,mode)) "#808080"
                     (if srgb "#839496" "#708183"))) ; (0.4406 0.5096 0.5169)
          (base1   (if (or (eq 'tlight ,mode) (eq 'tdark ,mode)) "#8a8a8a"
                     (if srgb "#93a1a1" "#81908f"))) ; (0.5060 0.5649 0.5636)
          (base2   (if (or (eq 'tlight ,mode) (eq 'tdark ,mode)) "#e4e4e4"
                     (if srgb "#eee8d5" "#e9e2cb"))) ; (0.9161 0.8900 0.7978)
          (base3   (if (or (eq 'tlight ,mode) (eq 'tdark ,mode)) "#ffffd7"
                     (if srgb "#fdf6e3" "#fcf4dc"))) ; (0.9894 0.9579 0.8641)
          (yellow  (if (or (eq 'tlight ,mode) (eq 'tdark ,mode)) "#af8700"
                     (if srgb "#b58900" "#a57705"))) ; (0.6475 0.4675 0.0235)
          (orange  (if (or (eq 'tlight ,mode) (eq 'tdark ,mode)) "#d75f00"
                     (if srgb "#cb4b16" "#bd3612"))) ; (0.7418 0.2133 0.0735)
          (red     (if (or (eq 'tlight ,mode) (eq 'tdark ,mode)) "#d70000"
                     (if srgb "#dc322f" "#c60007"))) ; (0.7770 0.0000 0.0290)
          (magenta (if (or (eq 'tlight ,mode) (eq 'tdark ,mode)) "#af005f"
                     (if srgb "#d33682" "#c61b6e"))) ; (0.7774 0.1080 0.4352)
          (violet  (if (or (eq 'tlight ,mode) (eq 'tdark ,mode)) "#5f5faf"
                     (if srgb "#6c71c4" "#5859b7"))) ; (0.3479 0.3514 0.7179)
          (blue    (if (or (eq 'tlight ,mode) (eq 'tdark ,mode)) "#0087ff"
                     (if srgb "#268bd2" "#2075c7"))) ; (0.1275 0.4627 0.7823)
          (cyan    (if (or (eq 'tlight ,mode) (eq 'tdark ,mode)) "#00afaf"
                     (if srgb "#2aa198" "#259185"))) ; (0.1468 0.5708 0.5250)
          (green   (if (or (eq 'tlight ,mode) (eq 'tdark ,mode)) "#5f8700"
                     (if srgb "#859900" "#728a05"))) ; (0.4498 0.5412 0.0202)
          (foregrounds (list base1 base0 base00 base01))
          (backgrounds (list base03 base02))
          (contrast-backgrounds (list base3 base2)))
     (when (or (eq 'tlight ,mode) (eq 'light ,mode))
...
(defun color-theme-sanityinc-solarized (mode)
  ...
  (if (fboundp 'load-theme)
      (let ((name (cond
                    ...
                    ((eq 'tlight mode) 'sanityinc-solarized-tlight)
                    ((eq 'tdark mode) 'sanityinc-solarized-tdark)
...
;;;###autoload
(defun color-theme-sanityinc-solarized-tdark ()
  "Apply the tdark solarized theme."
  (interactive)
  (color-theme-sanityinc-solarized 'tdark))

;;;###autoload
(defun color-theme-sanityinc-solarized-tlight ()
  "Apply the tlight solarized theme."
  (interactive)
  (color-theme-sanityinc-solarized 'tlight))
